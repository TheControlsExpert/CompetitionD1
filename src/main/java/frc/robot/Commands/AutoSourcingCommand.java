...
//         public AutoSourcingCommand(Drive drive, Superstructure superstructure, Intake Intake, DoubleSupplier xSupplier, DoubleSupplier ySupplier, DoubleSupplier rotationSupplier) {
//             this.drive = drive;
...
//         if (Math.hypot(Math.hypot(xSupplier.getAsDouble(), ySupplier.getAsDouble()), rotationSupplier.getAsDouble()) < 0.26 && !Intake.hasSensed() && shouldAutoAlign) {
...
//         if (Intake.hasSensed()) {
//           CommandScheduler.getInstance().schedule(new IntakeCommandArm(superstructure));
//         }
